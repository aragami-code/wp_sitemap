...
class S_Sitemaps extends S_Framework_V3
{
	/**
	 * Sitemap generation message logger
	 *
	 * @var S_Sitemaps_Logger_MessageLogger
	 */
	 ...
	 *
	 * @var S_Sitemaps_Logger_SitemapLogger
	 */
	 ...
	 *
	 * @var S_Sitemaps_Handler_AjaxHandler[]
	 */
		...
		S_Cache $cache = null)
	{
		...
		$this->add_option_key('S_MB_GENERATOR', 's_mb_generator',
			$this->bridge->t('XML Sitemaps', $this->domain));

		$this->add_option_key('S_MB_EXTENSIONS', 's_mb_extensions',
			$this->bridge->t('Google SiteMap', $this->domain));

		// @since 1.4.0 backward compat for google news option key
		if (!defined('S_MB_GOOGLE_NEWS'))
		{
			define('S_MB_GOOGLE_NEWS', 's_mb_google_news');
		}

		$this->add_extra_option_key('S_MB_GENERATOR_ADVANCED', 's_mb_generator_advanced',
			$this->bridge->t('Options', $this->domain));

		$this->add_option_key('S_MB_STATS', 's_mb_stats',
			$this->bridge->t('Log', $this->domain));

		if (!defined('S_MB_LOG'))
		{
			define('S_MB_LOG', 's_mb_log');
			define('S_MB_PING', 's_mb_ping_data');

			// @since 1.4.0 allow excluding posts/terms via admin page
			define('S_MB_EXCLUDED_POSTS', 's_mb_excluded_posts');
			define('S_MB_EXCLUDED_TERMS', 's_mb_excluded_terms');

			// @since 1.4.0 allow adding external pages via admin page
			define('S_MB_EXTERNAL_PAGES', 's_mb_external_pages');

			// @deprecated 1.4.0 use s_GXS_GENERATOR instead
			define('S_MB_OPTION_GENERATOR', 's_mb_generator');
		}

		$this->build_properties('S_MB', $options,
			dirname(dirname(__FILE__)) . '/s-mb.php',
			'', false
		);
	}
	...
	protected function pre_init_properties()
	{
		$this->templates = array(
			'credit' => '<!-- Generated by Mage Bazard XML Sitemaps %s (c) %s ' . "\n"
				. 'Plugin homepage: %s -->',
		...
		$this->post_excluder = new S_Sitemaps_Excluder(
			$this->bridge, $this->cache, 'excluded_posts', S_MB_EXCLUDED_POSTS
		);

		$this->term_excluder = new S_Sitemaps_Excluder(
			$this->bridge, $this->cache, 'excluded_terms', S_MB_EXCLUDED_TERMS
		);

		$this->providers = array(
			'post'          => new S_Sitemaps_Provider_Post($this, $this->post_excluder),
			'taxonomy'      => new S_Sitemaps_Provider_Taxonomy($this, $this->term_excluder),
			'external_page' => new S_Sitemaps_Provider_ExternalPage($this, S_MB_EXTERNAL_PAGES)
		);

		$this->ajax_handlers = array(
			'post'          => new S_Sitemaps_Handler_Ajax_PostHandler($this->get_provider('post')),
			'taxonomy'      => new S_Sitemaps_Handler_Ajax_TaxonomyHandler($this->get_provider('taxonomy')),
			'external_page' => new S_Sitemaps_Handler_Ajax_ExternalPageHandler(
				$this->get_provider('external_page'),
				$this->frequencies,
				$this->priorities,
				$this->get_current_timezone()
			),
			'news_terms' => new S_Sitemaps_Handler_Ajax_NewsTermsHandler(
	...
	protected function load_libraries()
	{
		...
		$this->sitemap_cache = new S_MB_CACHE($this);
	}
	...
	protected function pre_init_hooks()
	{
		...
		add_filter('s_mb_excluded_posts', array($this, 'add_excluded_posts'), 999, 3);
		add_filter('s_mb_excluded_terms', array($this, 'add_excluded_terms'), 999, 3);
		...
		add_filter('s_mb_external_pages', array($this, 'add_external_pages'), 999);
		...
		if (is_admin())
		{
			...
			add_action('wp_ajax_s-mb-get-posts', array($this->get_ajax_handler('post'), 'get_posts_action'));
			add_action('wp_ajax_s-mb-get-excluded-posts', array($this->get_ajax_handler('post'), 'get_excluded_posts_action'));
			add_action('wp_ajax_s-mb-remove-excluded-post', array($this->get_ajax_handler('post'), 'remove_excluded_item_action'));

			add_action('wp_ajax_s-mb-get-terms', array($this->get_ajax_handler('taxonomy'), 'get_terms_action'));
			add_action('wp_ajax_s-mb-get-excluded-terms', array($this->get_ajax_handler('taxonomy'), 'get_excluded_terms_action'));
			add_action('wp_ajax_s-mb-remove-excluded-term', array($this->get_ajax_handler('taxonomy'), 'remove_excluded_item_action'));

			add_action('wp_ajax_s-mb-get-external-pages', array($this->get_ajax_handler('external_page'), 'get_pages_action'));
			add_action('wp_ajax_s-mb-submit-external-page', array($this->get_ajax_handler('external_page'), 'save_external_page_action'));
			add_action('wp_ajax_s-mb-remove-external-page', array($this->get_ajax_handler('external_page'), 'remove_external_page_action'));

			add_action('wp_ajax_s-mb-get-object-taxonomies', array($this->get_ajax_handler('taxonomy'), 'get_taxonomies_action'));
			add_action('wp_ajax_s-mb-get-news-term-genres', array($this->get_ajax_handler('news_terms'), 'get_term_genres_action'));
	...
	protected function init_properties()
	{
		 ...
		 * ```
		 */
		$module_map       = $this->bridge->apply_filters('s_mb_module_mapping', array());
	...
	{
		$style_deps = array('s-option-page');

		if ($this->is_admin_page(S_MB_GENERATOR))
		{
			$style_deps = array('s-select2', 's-datatables', 's-jquery-ui', 's-option-page');

			$this->enqueue_media_file('s-mb-admin',
				S_MB_JS . '/s-mb-admin.js',
				array(
					's-select2',
					's-datatables',
					's-inputmask',
				...
				S_MB_DIST . '/js/script.min.js'
			);

			if ($this->is_admin_page(S_MB_GENERATOR))
			{
				wp_localize_script('s-mb-admin', 's_mb', array(
					'nonce' => array(
						'remove_excluded_item' => wp_create_nonce('s_mb_remove_excluded_item'),
						'remove_external_page' => wp_create_nonce('s_mb_manage_external_page')
					),
					'text'  => array(
						'exclude_items' => array(
		...
		elseif ($this->is_admin_page(S_MB_EXTENSIONS))
		{
			$style_deps = array('s-datatables', 's-option-page');

			$this->enqueue_media_file('s-mb-admin-extensions',
				S_MB_JS . '/admin-extensions.js',
				array(
					's-datatables',
					's-op'
				), false,
		...
		{
			$this->enqueue_media_file('s-mb-admin',
				S_MB_CSS . '/style.css', $style_deps, false,
				S_MB_DIST . '/css/style.min.css'
			);
		}
	}
	...
	public function insert_rewrite_rules($rules)
	{
		...
		/**
		 **/
		$custom_rules = apply_filters('s_mb_rewrite_rules', array());
	...
	{
		return defined('S_MB_CACHE_DIR') && S_MB_CACHE_DIR != ''
			? trim(S_MB_CACHE_DIR) : '';
	}
	...
	public function get_cache_directory()
	{
		 ...
		 */
		return $this->bridge->apply_filters('s_mb_cache_dir', $cache_dir);
	}
	...
	private function _init_module_directories()
	{
		...
		$this->custom_module_directory = $this->bridge->trailingslashit(
			$this->bridge->apply_filters('s_mb_module_dir', $this->custom_module_directory)
		);
	}
	...
	private function _get_non_permalink_query_var()
	{
		 ...
		 * Default to 'ssitemap'.
		 */
		return $this->bridge->apply_filters('s_mb_query_var_non_perma', 'ssitemap');
	}
	...
	private function _init_xslt_stylesheet()
	{
		...
		$this->xslt = !empty($this->options['input_custom_xslt'])
			...
			: $this->plugin_wp_url . 'assets/xsl/s-sitemap.xsl';
		 ...
		 * @since 1.3.0
		 */
		$this->xslt       = $this->bridge->apply_filters('s_mb_xslt', $this->xslt);
	...
	{
		$this->message_logger = S_Sitemaps_Logger::create_message_logger(25);
		$this->sitemap_logger = S_Sitemaps_Logger::create_sitemap_logger();

		// populate logger with logs currently stored in db
		$logs = $this->bridge->get_option(S_MB_LOG);
	...
	public function init_upgrade_plugin($from, $to)
	{
		...
		{
			$this->update_some_options(S_MB_GENERATOR, array(
				'input_cache_dir' => ''
			));
		}
	...
	protected function build_menus()
	{
		if (!empty($this->_menu_under_settings))
		{
			...
			add_options_page(
				__('MB XML Sitemaps', $this->domain),
				__('MB XML Sitemaps', $this->domain),
				S_MB_CAPABILITY,
				S_MB_GENERATOR,
				S_MB_GENERATOR_ADVANCED,
				S_MB_EXTENSIONS,
				S_MB_STATS,
		...
		else
		{
			...
			add_submenu_page(
				S_MB_GENERATOR,
				__('MB XML Sitemaps', $this->domain),
				__('Google Sitemaps', $this->domain),
				S_MB_CAPABILITY,
				S_MB_EXTENSIONS,
				array($this, 'show_option_pages')
			);
	...
	private function _disable_overridden_inputs(array &$inputs)
	{
		...
		{
			if (has_filter('s_mb_cache_dir') || $this->_get_cache_directory_from_constant())
			{
	...
	{
		/**$languages = include_once S_MB_PLUGIN_SRC . '/provider/google-news/languages.php';
		...
		return (array) $this->bridge->apply_filters('s_mb_news_languages', $languages);
	*/}
	...
	private function _get_formatted_sitemap_logs()
	{
		...
		/* @var $item s_Sitemaps_Logger_Sitemap_LogItem */
	...
	protected function build_option_page()
	{
		...
		if ($page == S_MB_GENERATOR)
		{
			...
			$form = array(
				...
				'heading' => array(
					...
					'heading_limit' => '<em>'
						...
						. sprintf(__('Setting too high limits might lead to '
							...
							. 'Refer to this plugin\'s  for more info.', $this->domain),
							$this->plugin_url . 'faq/')
						. '</em>',
					'heading_default' => '<em>'
				...
				'checkbox' => array(
					...
					'enable_exclude_posts_by_terms' => array(__('Also exclude posts that belong to excluded terms. When a post belongs to multiple terms, '
					...
					'enable_xslt'                   => array(__('Default XSLT stylesheets will be used. Set your custom stylesheets below or filter the <code>s_mb_xslt</code> hook.', $this->domain) => ''),
					...
					'enable_sitemap_site'           => array(__('Site Address For a multi-site installation of WordPress, '
				...
				'attributes' => array(
					'enable_sitemap_external' => array(
						'class'       => 's-switch-select s-switch-on-load',
						'data-target' => 'external-pages',
					),
					'select_exclude_post_type' => array(
						'class'               => 's-switch-select',
						'data-target'         => 'wrapper-exclude-posts',
						'data-callback-after' => 's_select_exclude_post_cb'
					),
					'select_exclude_taxonomy' => array(
						'class'               => 's-switch-select',
						'data-target'         => 'wrapper-exclude-terms',
						'data-callback-after' => 's_select_exclude_term_cb'
					),
					'enable_sitemap_split_post' => array(
						'class'       => 's-switch-on-load s-switch-select',
			...
			add_action('s_option_page_custom_action_exclude_posts', array($this, 'handle_exclude_posts'));
			add_action('s_option_page_custom_action_exclude_terms', array($this, 'handle_exclude_terms'));
			...
			// add extra forms
			add_action('s_option_action_after_form', array($this, 'add_external_page_modal'));

			// build options dynamically
			add_filter('s_option_page_submit_options', array($this, 'handle_dynamic_generator_options'));
		}
		elseif ($page == S_MB_EXTENSIONS)
		{
			...
			$form = array(
				...
				'input' => array(/**/
					'input_news_name' => array(
						'size'  => 70,
							'label' => __('Set a different name for your news sitemap. '
				...
				'checkbox' => array(
					'enable_image_sitemap' => array(__('Add an <code>&lt;image:image&gt;</code> entry to each sitemap item when possible. Please make sure you have enabled the '
								. 'Post Thumbnails feature in your theme before '
								. 'enabling this extension. Important', $this->domain) . ':</strong> '
							. __('This extension has an effect on performance, '
							. 'if you notice any slowdown please try disabling '
							. 'it first. Learn more about Image sitemaps.', $this->domain) => ''),
				...
				'post' => array(
					'select_news_cat_action' => '&nbsp;<em>'
						...
						. '<button type="button" class="button-secondary s-switch-button" '
							...
							. 'data-callback="s_button_view_selected_term_genres_cb"'
						. '>'
				...
				'attributes' => array(
					...
					'select_news_post_type' => array(
						...
						'data-callback' => 's_select_news_post_type_cb'
					),
					'select_news_taxonomy' => array(
						'class'         => 's-switch-select',
						'data-target'   => 'select_news_cat_action',
						'data-callback' => 's_select_news_taxonomy_cb'
					),
					'enable_news_keywords' => array(
						'class'       => 's-switch-on-load s-switch-select',
			...
			// build options dynamically
			add_filter('s_option_page_submit_options', array($this, 'handle_dynamic_image_sitemap_options'));
			add_filter('s_option_page_submit_options', array($this, 'handle_dynamic_google_news_options'));

			// handle option changes
			add_filter('s_option_page_action_submitted', array($this, 'handle_google_news_option_changes'));
		}
		elseif ($page == S_MB_GENERATOR_ADVANCED)
		{
			...
			$form = array(
				...
				'heading' => array(
					...
					'heading_cache' => '<em>'
						...
						. '</em>',
					'heading_robot' => '<em>'
						. sprintf(__('WordPress generates a virtual robots.txt '
				...
				'checkbox' => array(
					...
					'enable_log'            => array(sprintf(__('Log useful messages when sitemaps are generated. The log can be viewed <a href="%s">here</a>.', $this->domain), $this->get_admin_page_url(S_MB_STATS)) => ''),
					...
					'enable_debug_extra'    => array(sprintf(__('When this is on, NO headers are sent and sitemaps are NOT compressed, useful when debugging <em>Content Encoding Error</em>..', $this->domain), $this->plugin_url) => ''),
				),
				'input' => array(
			...
			add_filter('s_option_page_submit_form_name', create_function('', "return S_MB_GENERATOR;"));
		}
		elseif ($page == S_MB_STATS)
		{
			...
			if ('yes' != $this->options['enable_log'] || $this->message_logger->is_empty())
			{
				...
				add_filter('s_feed_showable', create_function('', 'return "";'));
				add_filter('s_option_submit_button', create_function('', 'return "";'));
			}
			else
			{
				...
				add_filter('s_option_submit_button', array($this, 'add_clear_log_button'));
				add_action('s_option_page_custom_action_clear_log', array($this, 'handle_clear_log_action'));
			}
		}
		...
		if (self::can_update_site_option()
			&& (S_MB_GENERATOR == $page || S_MB_GENERATOR_ADVANCED == $page)
		) {
			...
			add_filter('s_option_submit_button', array($this, 'add_flush_cache_buttons'));
			add_action('s_option_page_custom_action_flush_cache', array($this, 'handle_flush_action'));
			add_action('s_option_page_custom_action_save_flush_cache', array($this, 'handle_save_flush_action'));
		}
		...
		if ($this->current_options['enable_news_sitemap'] != $this->options['enable_news_sitemap']
			...
			if (!$is_news_sitemap_ok)
			{
				$this->add_error_flash(sprintf(
					...
					$this->get_admin_page_url(S_MB_STATS)
				));
			}
		}
	}
	...
	private function _handle_exclude_items($group, array $items_to_exclude, S_Sitemaps_Excluder $excluder)
	{
	...
	{
		if (! ($post_type = S_Framework_Util::get_request_var('select_exclude_post_type')))
			return;

		if (! ($posts_to_exclude = S_Framework_Util::get_request_var('select-exclude-posts')))
		{
	...
	{
		if (! ($taxonomy = S_Framework_Util::get_request_var('select_exclude_taxonomy')))
			return;

		if (! ($terms_to_exclude = S_Framework_Util::get_request_var('select-exclude-terms')))
		{
			$this->add_error_flash(__('Please select at least one item to exclude.', $this->domain));
			return;
		}
	...
	{
		return s_mb_format_header_time($time);
	}
	...
	{
		wp_die(__('<strong>MB XML Sitemaps Error:</strong> ', $this->domain)
			. $message, __('MB XML Sitemaps Error', $this->domain),
	...
	{
		$this->bridge->update_option(S_MB_LOG, array(
	...
	private function _log_message_item($message, $type, $time = null)
	{
		...
		$item = new S_Sitemaps_Logger_Message_LogItem($message, $type, $time);
	...
	{
		if (isset($sitemap))
			_deprecated_argument(__FUNCTION__, 'MB XML  Sitemaps');
	...
	{
		_deprecated_function(__FUNCTION__, 'MB XML Sitemaps', 'S_Sitemaps::log_error');
	...
	{
		$this->log_message($message, S_Sitemaps_Logger_Message_LogItem::TYPE_ERROR);
	...
	{
		_deprecated_function(__FUNCTION__, 'MB XML Sitemaps', 'S_Sitemaps::log_success');
	...
	{
		$this->log_message($message, S_Sitemaps_Logger_Message_LogItem::TYPE_SUCCESS);
	}

	public function nlog($message)
	{
		_deprecated_function(__FUNCTION__, 'MB XML Sitemaps', 'S_Sitemaps::log_notice');
	...
	{
		$this->log_message($message, S_Sitemaps_Logger_Message_LogItem::TYPE_NOTICE);
	}

	public function smlog($url)
	{
		_deprecated_function(__FUNCTION__, 'MB XML Sitemaps ', 'S_Sitemaps::log_sitemap');
	...
	private function _log_sitemap_item($url, $time = null)
	{
		...
		$item = new S_Sitemaps_Logger_Sitemap_LogItem($url, $time);
		$item->set_datetimezone($this->get_current_timezone());
	...
	public function add_post_title_like_query_variable($where, WP_Query $wp_query)
	{
		...
		if ($post_title_like = $wp_query->get('s_post_title_like'))
		{
	 ...
	 * the `s_gxs_modules_built` action hook.
	 *
	 * @example hooks/action_s_gxs_modules_built.php 2
	 *
	...
	private function _build_sitemap_modules()
	{
		...
		/**
		 */
		do_action('s_mb_modules_built', $this->modules, $this->post_types, $this->taxonomies);
			...
			if ($sub_module == 'google_news')
			{
				if (!empty($this->options['input_news_age']))
				{
					return sprintf(
						...
						. 'MB Sitemaps >> Google Sitemaps >> Google News Sitemap >> Sitemap Contents', $this->domain),
	...
	public function request_sitemap($wp_query)
	{
		...
		{
			// non-friendly sitemap url is used, i.e. http://example.com/?ssitemap=xxx
	...
	/**
	 * Checks whether requested sitemap is a s sitemap
	 *
	 * @since 1.3.0
	 * @access private
	 */
	private static function _is_s_sitemap($sitemap_name)
	{
	...
	private function _load_sitemap_from_cache($module_name, $sitemap_name)
	{
		...
		elseif (!empty($_GET['generate']) && current_user_can('manage_options'))
		{
			...
			// s_GXS_CACHE::get_cache_status(). This should be fixed in
			// future versions.
			return false;
		}
	...
	/**
	 * @param s_GXS_MODULE $module
	 * @return s_Sitemaps_Sitemap
	 */
	private function create_sitemap_from_module(S_MB_MODULE $module)
	{
		$sitemap_provider  = new S_Sitemaps_Sitemap_Provider($this, $module);
		$sanitizer_factory = new S_Sitemaps_Sitemap_Sanitizer_Factory($this);
		$sitemap_xsl       = null;

		switch ($module->get_type())
		{
			case 'url':
				$sitemap_class = 'S_Sitemaps_Sitemap_Xml';
				$sitemap_xsl   = $this->xslt;
				break;

			case 'news':
				$sitemap_class = 'S_Sitemaps_Sitemap_XmlGoogleNews';
				break;

			case 'index':
				$sitemap_class = 'S_Sitemaps_Sitemap_XmlIndex';
	...
	private function _load_sitemap_module($module, $sub_module = '')
	{
		...
		if (!self::_is_s_sitemap($sitemap_name))
		{
			// not a s sitemap, return this handle to WordPress
			return false;
		}
		...
		if ('sitemapindex' != $sitemap_name)
		{
			...
			$class_name = 'S_MB_MODULE_' . str_replace('-', '_', $module_name);
		...
		if ($this->sitemap->has_items())
		{
			...
			$this->_send_headers(array(
				...
				'expires' => $expires,
				'etag'    => md5($expires . s_mb_get_filename($sitemap_name))
			));
		...
		else
		{
			...
			$module_guide = 'google_news' != $this->module_data['sub_module']
				? __('Enable/disable sitemaps via <em>MB Sitemaps >> XML Sitemaps</em>.', $this->domain)
				: '';
			...
			$error_message_admin = $this->module_data['sitemap_name'] == 'sitemapindex'
				? ' ' . __('Please make sure that you have at least one sitemap enabled '
					. 'in <em>MB Sitemaps >> XML Sitemaps >> Sitemaps to generate</em>.', $this->domain)
	...
	public function ping($post)
	{
		...
		$ping_data = $this->bridge->get_option(S_MB_PING);
		...
		$this->bridge->update_option(S_MB_PING, $ping_data);

		$this->commit_logs();
	}

	/**
	 * @param S_Sitemaps_Excluder $excluder
	 */
	public function set_post_excluder(S_Sitemaps_Excluder $excluder)
	{
	 ...
	 * @param S_Sitemaps_Excluder $excluder
	 */
	public function set_term_excluder(S_Sitemaps_Excluder $excluder)
	{
	 ...
	 *
	 * @param string $name
	 * @return S_Sitemaps_Handler_AjaxHandler
	 */
	...
	public function get_news_name()
	{
		 ...
		 */
		return $this->bridge->apply_filters('s_mb_news_name', $news_name);
	}
}
