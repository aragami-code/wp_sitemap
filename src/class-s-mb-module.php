...
class S_MB_MODULE
{
	...
	protected function get_sitemap_url($slug)
	{
		...
		return $s_mb->get_sitemap_url($slug);
	}
	...
	protected function cal_frequency($item = null, $lastmod = null)
	{
		...
		if (empty($lastmod))
		{
			$freq = $s_mb->options['select_default_freq'];
		}
		else
		{
		 ...
		 */
		return apply_filters('s_mb_freq', $freq, $item);
	}
	...
	protected function cal_priority($item, $freq = 'daily')
	{
		...
		$score = $score < $s_mb->options['select_min_pri']
			? $s_mb->options['select_min_pri']
			: $score;
		 ...
		 */
		return apply_filters('s_mb_priority_score', $score, $item, $freq);
	}
	...
	protected function get_datetime_from_post($post, $publish = false)
	{
		...
		$current_timezone = $s_mb->get_current_timezone();
		...
		try {
			...
			$datetime->setTimezone(
				$s_mb->options['enable_gmt'] == 'yes' ? $utc_timezone : $current_timezone
			);
		} catch (Exception $e) {
			return '';
		}
	...
	protected function format_datetime($datetime)
	{
		...
		try {
			...
			// need local timezone
			if ($s_mb->options['enable_gmt'] != 'yes') {
				$datetime->setTimezone($s_mb->get_current_timezone());
			}
	 ...
	 *
	 * @deprecated 1.4.0 use s_GXS_MODULE::format_datetime() instead
	 *
	...
	protected function get_results($query_str)
	{
		...
		$end   = (int) $s_mb->options['input_sql_limit'];
	...
	protected function query_posts($query_str)
	{
		...
		{
			$query_str['posts_per_page'] = (int) $s_mb->options['input_sql_limit'];
	...
	protected function build_data()
	{
		...
		$split_limit = empty($s_mb->options['input_split_limit_post'])
			? $s_mb->options['input_item_limit']
			: $s_mb->options['input_split_limit_post'];
	...
	public function is_image_allowed()
	{
		...
		$post_type = $this->type == 'news'
			? $s_mb->options['select_news_post_type']
			: $this->requested;
		...
		$this->image_allowed = $s_mb->is_image_sitemap_allowed_for($post_type);
