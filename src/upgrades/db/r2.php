...
$old_logs = $this->bridge->get_option(S_MB_LOG);
	...
	foreach ($logs as $log) {
		...
		if ($key == 'log') {
			$new_logs['messages'][] = array(
				'message' => $log['log'],
				'type'    => $log['error'] === true
					? S_Sitemaps_Logger_Message_LogItem::TYPE_ERROR
					: ($log['error'] === 'notice'
						? S_Sitemaps_Logger_Message_LogItem::TYPE_NOTICE
						: S_Sitemaps_Logger_Message_LogItem::TYPE_SUCCESS),
...
if ($need_update) {
	$this->bridge->update_option(S_MB_LOG, $new_logs);
}
