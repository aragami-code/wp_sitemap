 ...
 */
class S_Sitemaps_Sitemap_Xml extends S_Sitemaps_Sitemap
{
	/**
	 * {@inheritDoc}
	 */
	...
	{
		$url = S_Sitemaps_Sitemap_Tag::create_compound_tag('url');
	 ...
	 * @param array S_Sitemaps_Sitemap_Sanitizer[] $sanitizers default to null
	 *
	 * @return array
	 */
	protected function create_tags_from_sitemap_item(
		...
		foreach ($tag_names as $tag_name => $item_key) {
			...
			$tags[] = S_Sitemaps_Sitemap_Tag::create_single_tag(
				$tag_name,
				$item[$item_key],
	 ...
	 *
	 * @param array $item
	 * @return mixed S_Sitemaps_Sitemap_Tag_CompoundTag|null
	 */
	protected function create_image_tag_from_sitemap_item(array $item)
	{
		...
		$image_tag = S_Sitemaps_Sitemap_Tag::create_compound_tag('image:image')
			->add_tags($tags);
