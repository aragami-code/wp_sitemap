 ...
 * 
 */
class S_Sitemaps_Handler_Ajax_ExternalPageHandler extends S_Sitemaps_Handler_AjaxHandler
{
	protected $provider;
	...
	public function __construct(
		S_Sitemaps_Provider_ExternalPage $provider,
		array $frequencies,
		array $priorities,
	...
	{
		$this->bridge->check_ajax_referer('s_mb_manage_external_page');
		...
		foreach ($required as $required_field) {
			if (! $values[$required_field] = S_Framework_Util::get_request_var($required_field)) {
		...
		if (! S_Sitemaps_Validator_Url::validate($url)
	...
	public function remove_external_page_action()
	{
		...
		if ($url = S_Framework_Util::get_request_var('url')) {
			// no matching page
