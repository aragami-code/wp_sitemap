...
class S_MB_MODULE_TAXONOMY extends S_MB_MODULE
{
	...
	protected function generate_data()
	{
		 ...
		 *
		 * @example hooks/filter_s_gxs_excluded_terms.php 2
		 *
		 ...
		 * @since 1.3.0
		 */
		$excluded_term_ids = apply_filters('s_mb_excluded_terms', array(), $requested);

		$excluded_term_ids_sql = count($excluded_term_ids) > 0
			? ' AND t.term_id NOT IN (' . implode(',', array_map('intval', $excluded_term_ids)) . ') ' : '';

		// @deprecated 1.3.0, use `s_gxs_excluded_term_slugs` instead
		$excluded_terms = apply_filters('s_mb_term_exclude', array(), $requested);

		/**
		 * Filter terms that are added to a taxonomy-based sitemaps using their slugs.
		 *
		 * @example hooks/filter_s_gxs_excluded_term_slugs.php 2
		 *
		 ...
		 * @since 1.4.0
		 */
		$excluded_term_slugs = apply_filters('s_mb_excluded_term_slugs', $excluded_terms, $requested);
