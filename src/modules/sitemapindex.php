...
class S_MB_MODULE_INDEX extends S_MB_MODULE
{
	...
	private function _get_sitemap_lastmod($sitemap_name)
	{
		...
		if ($sitemap_log = $s_mb->get_sitemap_logger()->get_sitemap_log_item($sitemap_name))
		...
		$sitemap_file = s_mb_get_filename($sitemap_name);
	...
	protected function build_data()
	{
		...
		if ('yes' == $s_mb->options['enable_sitemap_split_post'])
		{
			...
			foreach ($post_types as $post_type_name)
			{
				$excluded_posts = array_merge(
					$excluded_posts,
					apply_filters('s_mb_excluded_posts', array(), $post_type_name)
				);
			}
		...
		foreach ($this->requested_modules as $item)
		{
				...
				if ('yes' == $s_mb->options['enable_sitemap_split_post']
					...
					if (1 < $num_part)
					{
						...
						for ($i = 1; $i <= $num_part; $i++)
						{
							 ...
							 */
							$data['lastmod'] = apply_filters('s_mb_sitemap_lastmod', $data['lastmod'], $lastmod, $item, $i);
			...
			if (false == $passed)
			{
				...
				$data['lastmod'] = $lastmod ? $this->format_local_datetime($lastmod) : '';
				$data['lastmod'] = apply_filters('s_mb_sitemap_lastmod', $data['lastmod'], $lastmod, $item, 0);
		 ...
		 * @since 1.4.0
		 */
		$external_sitemaps = (array) apply_filters('s_mb_external_sitemaps', array());
