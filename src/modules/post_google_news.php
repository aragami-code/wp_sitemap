...
class S_MB_MODULE_POST_GOOGLE_NEWS extends S_MB_MODULE_POST
{
	...
	private static function map_keyword($keyword)
	{
		 ...
		 */
		$keywords_map = apply_filters('s_mb_news_keyword_map', array());
	...
	private function get_genres_from_post($post)
	{
		...
		$genres_cache = array();
		$news_genres  = $s_mb->options['input_news_genres'];
	...
	private function get_keywords_from_post($post)
	{
		...
		if ('yes' != $s_mb->options['enable_news_keywords'])
			return;

		$news_taxonomy  = $s_mb->options['select_news_taxonomy'];

		$keywords       = array();
		$keyword_source = $s_mb->options['select_news_keyword_source'];
	...
	{
		global $wpdb, $post, $s_mb;

		$lang = $s_mb->options['select_news_lang'];

		// @since 1.4.0 support custom post type for google news sitemap
		$news_post_type = $s_mb->options['select_news_post_type'];
		$news_taxonomy  = $s_mb->options['select_news_taxonomy'];

		$news_terms       = $s_mb->options['select_news_cats'];
		$news_term_action = $s_mb->options['select_news_cat_action'];
		...
		if ($news_terms)
		{
			...
			$term_query = $news_term_action != 'inc'
				&& $s_mb->options['enable_news_multicat'] == 'yes'
				? '' : $term_query;
		}

		// publishing date restriction
		$publishing_date_query = !empty($s_mb->options['input_news_age'])
			? $wpdb->prepare('AND p.post_date_gmt > %s', self::news_time($s_mb->options['input_news_age']))
			: '';

		$group_by = empty($s_mb->options['enable_news_multicat'])
			? ' GROUP BY p.ID' : '';
		...
		if ('yes' == $s_mb->options['enable_news_multicat'])
		{
