...
class S_MB_CACHE
{
	...
	public function __construct(S_Sitemaps $main)
	{
	...
	private function _check_cache()
	{
		...
		$lastmod = s_mb_format_header_time($last_modified);
		$expires = s_mb_format_header_time($last_modified + $this->cache_time);
	...
	public function get_cache_status($module_name, $sitemap_name)
	{
		...
		$this->cache_file = s_mb_get_filename($sitemap_name);
