...
class S_Option_Page_V3
{
	 ...
	 * @param S_Framework_V3 $plugin
	 */
	public function __construct($form_name, S_Framework_V3 $plugin)
	{
	...
	{
		$return_str  = '<div class="wrap s-wrap" style="padding-bottom: 20px;">' . "\n";
		$return_str .= '<h1 style="display: none;">' . $this->plugin->plugin_title . '</h1>' . "\n";
		echo $return_str;

		do_action('s_option_action_before_main');

		$return_str = '<div id="s-main">' . "\n";
		echo $return_str;

		do_action('s_option_action_before_tabs');

		$return_str = '';

		// @since rev 164 always show tabs, even when there's only one
		$count = 0;
		$return_str .= '<h2 class="nav-tab-wrapper s-option-page-tabs">' . "\n";
		...
		$return_str .= apply_filters('s_option_before_form', '');
		echo $return_str;

		do_action('s_option_action_before_form');

		$return_str  = '';
		$return_str .= '<form class="s-option-page" name="' . $this->form_name . '" method="post" action="">'  . "\n";
		...
		$return_str .= '</ul>' . "\n";
		$return_str .= apply_filters('s_option_before_submit_button', '');

		echo $return_str;
		do_action('s_option_action_before_submit_button');

		$return_str  = '';
		$return_str .= apply_filters('s_option_submit_button',
		...
		echo $return_str;
		do_action('s_option_action_after_form');
	...
	public function register_custom_submit_action($action, $callback = null)
	{
		if (!is_null($callback))
		{
			...
			$this->bridge->add_filter('s_option_page_custom_action_' . $action, $callback, 10, 2);
		}
	...
	public function submit_html_form()
	{
		...
		// form name
		$form_name = $this->bridge->apply_filters('s_option_page_submit_form_name', $this->form_name);

		// save $_POST options for later use
		$post_options = $options;

		// allow filtering the options that are going to be updated
		$options = $this->bridge->apply_filters('s_option_page_submit_options', $options);
	...
	public function handle_form_actions()
	{
		...
		if (isset($_POST['submit_' . $this->get_form_name()]))
		{
			...
			if ($this->submit_html_form())
			{
				...
				$redirect = $this->bridge->apply_filters('s_option_page_action_submitted', true);
		...
		else
		{
			foreach ($this->form_actions as $action)
			{
				if (isset($_POST[$action]))
				{
					...
					$redirect = $this->bridge->apply_filters('s_option_page_custom_action_' . $action, true, $action);
	...
	protected function generate_field_help_attributes($name, array &$attributes = array())
	{
		...
		switch ($help_data['type'])
		{
			...
			case 'hover':
			case 'switch':
				$help_class = 's-popover-' . $help_data['type'];
				break;
		}
		...
		elseif ($help_data['type'] == 'block')
		{
			$post_html = $help_data['content']
				? '<span class="s-form-help-block">' . $help_data['content'] . '</span>'
				: '';
		...
		switch ($type)
		{
			...
			case 'help':
				...
				else
				{
					...
					$html_field = '<a class="s-field-help-link" target="_blank" '
			...
			foreach ($data as $key => $value)
			{
				if ($type == 'checkbox')
				{
					...
					if ($this->form_options[$name] == 'yes')
					{
						...
						$return_html .= apply_filters('s_option_after_' . $type . '_' . $name . '_checked', '', $value, $param);
						$return_html .= $br;
					}
					else
					{
						...
						$return_html .= apply_filters('s_option_after_' . $type . '_' . $name, '', $value, $param);
						$return_html .= $br;
					}
				}
	...
	protected function generate_html_fields($type, $name)
	{
		...
		$input_class = strpos($type, 'heading') === 0
			? 's-option-page-heading-desc'
			: 's-option-page-inputs';
		...
		switch ($type)
		{
			case 'section':
				...
				$item_label_html = '<span class="s-opton-page-label">'
					. $item_label
					. $inline
					. '</span>';
			...
			default:
				...
				$item_label_html = $type != 'checkbox' && $type != 'checkbox_multi' && $type != 'radio'
					? '<label class="s-opton-page-label type-' . $type . '" for="' . $name . '">'
						. $item_label
						. $inline
						. '</label>'
					: '<span class="s-opton-page-label type-' . $type . '">'
						. $item_label
						. $inline
						. '</span>';
	...
	protected function is_form_item_hidden($name)
	{
		...
		if (isset($this->form['php'])
			...
			&& !S_Version::get_current_php_version_id($this->form['php'][$name])
		) {
		...
		if (isset($this->form['role'])
			...
			&& (!S_Framework_V3::is_site_admin() || !S_Framework_V3::is_on_main_blog())
		) {
		...
		if (isset($this->form['blog'])
			...
			if ($this->form['blog'][$name] == 'main' && !S_Framework_V3::is_on_main_blog())
			{
				// this field should be on main blog only
				return true;
			}
			elseif ($this->form['blog'][$name] == 'sub' && S_Framework_V3::is_on_main_blog())
			{
