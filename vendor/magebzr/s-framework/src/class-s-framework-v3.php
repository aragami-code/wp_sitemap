...
abstract class S_Framework_V3
{
	 ...
	 *
	 * @param array $meta
	 * @param S_WP_Bridge $bridge optional, default to null
	 * @since rev 142
	 */
		...
		S_Cache $cache = null)
	{
		...
		$this->set_version(isset($meta['php_version']) ? $meta['php_version'] : S_Version::$php_ver, 'php');
		$this->set_version(isset($meta['wp_version']) ? $meta['wp_version'] : S_Version::$wp_ver, 'wp');
		$this->set_version($meta['version']);

		$this->domain = $meta['domain'];

		$this->bridge = $bridge ? $bridge : new S_WP_Bridge();
		$this->cache  = $cache  ? $cache  : new S_Cache($this->bridge, $this->plugin_key);
	}
	...
	{
		return '<div class="icon32" id="icon-s-plugin" '
	...
	{
		return S_Version::get_current_php_version($version);
	}
	...
	{
		S_Version::warn_required_versions($this->plugin_title, $this->domain, $this->php_ver, $this->wp_ver);
	}
...
<?php
	...
	{
		$sidebar_showable = $this->bridge->apply_filters('s_info_showable', true); // @deprecated rev 164
		$sidebar_showable = $this->bridge->apply_filters('s_sidebar_showable', $sidebar_showable); // @since rev 164
		$feed_showable    = $this->bridge->apply_filters('s_feed_showable', true); // @since rev 164
		$ad_showable      = $this->bridge->apply_filters('s_ad_showable', true);
...
<?php
	...
	protected function pre_init_actions()
	{
		...
		// 1. Other plugins can hook to s hooks and can compete with s
	...
	public function build_wp_properties()
	{
		...
		if (defined('S_USE_SYMLINKS'))
			...
			// this should allow other package to include S plugins while
	...
	public function init()
	{
		...
		if ($this->is_admin_page())
		{
			...
			// page by default
			$this->bridge->wp_enqueue_script('s-op');

			// @since rev 164 split the sidebar
			$this->bridge->add_action('s_option_action_before_main', array($this, 'show_sidebar_right'), 12);
			$this->bridge->add_action('s_option_action_before_tabs', array($this, 'show_header'), 12);
		}
	...
	{
		if (! S_Framework_Util::is_debugging())
		{
			$prod_src = $prod_src
				? $prod_src
				: str_replace(
	...
	{
		if ($prod_handle && ! S_Framework_Util::is_debugging())
		{
	...
	{
		$asset_url = $this->plugin_wp_url . 'vendor/magebzr/s-framework/assets';

		// select2
		$this->register_media_file('s-placeholders',
			$asset_url . '/vendor/placeholders/placeholders.jquery.js',
			array(), $this->revision,
			$asset_url . '/vendor/select2/js/select2.min.js',
			's-select2'
		);
		$this->register_media_file('s-select2',
			$asset_url . '/vendor/select2/js/select2.js',
			array('s-placeholders'), $this->revision,
			$asset_url . '/vendor/select2/js/select2.min.js',
			's-select2'
		);
		$this->register_media_file('s-select2',
		...
		// datatables
		$this->register_media_file('s-datatables',
		...
		// bootstrap
		$this->register_media_file('s-bootstrap',
			$asset_url . '/vendor/bootstrap/js/bootstrap.js',
			array('jquery'), $this->revision
		);

		// bootstrap modal
		$this->register_media_file('s-bootbox',
			$asset_url . '/vendor/bootbox.js/bootbox.js',
			array('s-bootstrap'), $this->revision,
			$asset_url . '/option-page/dist/js/modal.min.js',
			's-op-modal'
		);
		$this->register_media_file('s-op-bootbox',
			$asset_url . '/option-page/js/bootbox.js',
			array('jquery', 's-bootbox'), $this->revision,
			$asset_url . '/option-page/dist/js/modal.min.js',
			's-op-modal'
		);
		$this->register_media_file('s-op-modal',
			$asset_url . '/option-page/js/modal.js',
			array('s-op-bootbox', 's-op-common'),
			$this->revision,
			$asset_url . '/option-page/dist/js/modal.min.js',
			's-op-modal'
		);
		...
		$this->register_media_file('s-codemirror-css',
			$asset_url . '/vendor/codemirror/mode/css/css.js',
			array('s-codemirror'), $this->revision,
			$asset_url . '/option-page/dist/js/code-editor.min.js',
			's-op-codemirror'
		);
		$this->register_media_file('s-codemirror-addon-placeholder',
			$asset_url . '/vendor/codemirror/addon/display/placeholder.js',
			array(), $this->revision,
			$asset_url . '/option-page/dist/js/code-editor.min.js',
			's-op-codemirror'
		);
		$this->register_media_file('s-codemirror-addon-active-line',
			$asset_url . '/vendor/codemirror/addon/selection/active-line.js',
			array(), $this->revision,
			$asset_url . '/option-page/dist/js/code-editor.min.js',
			's-op-codemirror'
		);
		$this->register_media_file('s-codemirror-htmlmixed',
			$asset_url . '/vendor/codemirror/mode/htmlmixed/htmlmixed.js',
			array('s-codemirror'), $this->revision,
			$asset_url . '/option-page/dist/js/code-editor.min.js',
			's-op-codemirror'
		);
		$this->register_media_file('s-op-codemirror',
		...
		// s common
		$this->register_media_file('s-op-common',
	...
	public function init_admin_page()
	{
		...
		$this->current_option_page = new S_Option_Page_V3(
	...
	{
		//$this->_menu_under_settings = $this->bridge->apply_filters('s_menus_under_settings', false);
		...
		if ($this->is_admin_page())
		{
			...
			$asset_url = $this->plugin_wp_url . 'vendor/magebzr/s-framework/assets/option-page';

			// enqueue style sheets and scripts for the option page
			$this->enqueue_media_file('s-option-page',
			...
			$this->enqueue_media_file('s-paypal-js',
	...
	public function add_container_flash($field, $message)
	{
		...
		$flash_key = 's_op_flash_container';
...
<?php
	...
	public function add_notice($notice)
	{
		...
		{
			$this->notices[] = $notice;
			$this->bridge->add_action('s_option_action_before_form', array($this, 'show_notices'));
		}
	}
...
<?php
	...
	public function add_error($error)
	{
		...
		{
			$this->errors[] = $error;
			$this->bridge->add_action('s_option_action_before_form', array($this, 'show_errors'));
		}
	}
...
<?php
	...
	{
		return S_Framework_Util::is_multisite();
	}

	public static function is_subdomain_install()
	{
		return S_Framework_Util::is_subdomain_install();
	}

	public static function is_super_admin()
	{
		return S_Framework_Util::is_super_admin();
	}

	public static function is_site_admin()
	{
		return S_Framework_Util::is_site_admin();
	}

	public static function is_multisite_admin()
	{
		return S_Framework_Util::is_multisite_admin();
	}

	public static function is_on_main_blog()
	{
		return S_Framework_Util::is_on_main_blog();
	}

	public static function can_update_site_option()
	{
		return S_Framework_Util::can_update_site_option();
	}

	public static function is_apache()
	{
		return S_Framework_Util::is_apache();
	}

	public static function is_nginx()
	{
		return S_Framework_Util::is_nginx();
	}
