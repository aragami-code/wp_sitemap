...
jQuery(function($) {
	...
	function toggle_field($t) {
		...
		// contained in a standard s form item
		var $target = ! $('#' + target_id).is(':input') && ! $('#' + target_id).is('h3')
			? $('#' + target_id) : $('#' + target_id).parents('.s-clear');

		// no toggle value, just show the target immediately when the selected
		// value is not blank
		if (!toggle_value) {
			if ($t.is(':button')) {
				$target.toggleClass('s-no-display');
			} else if ($t.is(':checkbox')) {
				var cb_invert = $t.data('checkboxInvert');
				if (cb_invert) {
					$target.toggleClass('s-no-display', $t.prop('checked'));
				} else {
					$target.toggleClass('s-no-display', ! $t.prop('checked'));
				}
			} else {
				$target.toggleClass('s-no-display', '' === current_value);
			}
		} else if (!$t.is(':button')) {
			$target.toggleClass('s-no-display', toggle_value !== current_value);
		}
	...
	$('.s-switch-select').on('change', function(e) {
		handle_switch_action($(this));
		e.preventDefault();
	});

	// switch by on click event
	$('.s-switch-button').on('click', function(e) {
		handle_switch_action($(this));
		e.preventDefault();
	});

	// switch on first load
	$('.s-switch-on-load').each(function() {
		toggle_field($(this));
	});
});
