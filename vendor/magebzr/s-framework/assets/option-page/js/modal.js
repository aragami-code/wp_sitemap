/*global jQuery,ajaxurl,s_common*/
jQuery(function($) {
	'use strict';

	var message_timeout;

	$('body').on('click', '.s-button-modal', function(e) {
		e.preventDefault();
		...
		if (typeof ajax_action === 'undefined') {
			...
			$.get(ajaxurl, {
				...
				$(target_id)
					.find('.s-modal-content')
					.html(r);

				s_common.enhance_form_fields($(target_id));
	...
	$('.s-modal').on('submit', 'form', function(e) {
		e.preventDefault();

		var $form = $(this);
		var $btn  = $form.parents('.s-modal').find('.s-button-modal-submit');
		...
		var $loader = $btn
			.parents('.s-modal-footer')
			.find('.s-modal-message');
		...
		$.post(ajaxurl, data, function(r) {
			...
			if ($btn.parents('.s-modal-footer').find('.s-button-modal-reset').length === 0) {
				$form.trigger('reset');
			}
	...
	$('.s-modal').on('click', '.s-button-modal-submit', function(e) {
		e.preventDefault();
	...
	$('.s-modal').on('click', '.s-button-modal-reset', function(e) {
		e.preventDefault();
	...
	$('.s-modal').on('show.bs.modal', function(e) {
		var $t = $(this);

		$t.find('form')
			.trigger('reset');

		$t.find('.s-modal-message')
			.removeClass('text-success text-danger')
			.hide();
	});

	// vertically center any modal
	$('body').on('shown.bs.modal', '.s-modal', function(e) {
		var $dialog = $(this).find('.s-modal-dialog');
