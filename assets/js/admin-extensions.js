...
function s_disable_inputs($) {
	...
	$('input[name="submit_s_mb_google_news"]').prop('disabled', true);
}

function s_enable_inputs($) {
	...
	$('input[name="submit_s_mb_google_news"]').prop('disabled', false);
}

function s_select_news_post_type_cb($, $t) {
	var post_type = $t.val();

	s_disable_inputs($);

	$.get(ajaxurl, {
		action: 's-mb-get-object-taxonomies',
		post_type: post_type
	}, function(r) {
		if ($.isArray(r)) {
...
function s_select_news_taxonomy_cb($, $t, hide_loader_cb) {
...
function s_button_view_selected_term_genres_cb($, $t, hide_loader_cb) {
	// init datatable if not already done so
	if (! $.fn.dataTable.isDataTable('#table-selected-term-genres')) {
		$.get(ajaxurl, {
			action: 's-mb-get-news-term-genres',
